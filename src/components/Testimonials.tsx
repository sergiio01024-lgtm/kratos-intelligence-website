    ...
    <section className="py-20 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl lg:text-4xl mb-4">What Clients Say</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {testimonials.map((testimonial, index) => (
            <FadeIn key={index} delay={index === 0 ? 0 : 0.15}>
              <div className="p-6 rounded-2xl border border-border bg-muted/20 h-full">
                <div className="text-4xl text-muted-foreground/30 font-serif leading-none mb-2">"</div>
                <p className="text-sm leading-relaxed text-foreground mb-4">
                  {testimonial.quote}
                </p>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-medium text-sm ${testimonial.avatarClass}`}>
                    {testimonial.initial}
                  </div>
                  <div>
